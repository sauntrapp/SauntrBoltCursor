...
export default function SauntrMapView({ 
        ...
        >
          ...
          {markers.map((marker) => (
            <Marker
              key={marker.venue.id}
              coordinate={{
                latitude: marker.venue.latitude,
                longitude: marker.venue.longitude,
              }}
              title={marker.venue.name}
              description={marker.venue.address}
              onPress={() => handleMarkerPress(marker.venue)}
            >
              <View style={[
                styles.realMarkerPin,
                marker.type === 'instaplan' && styles.instaplanRealMarker,
                marker.isSelected && styles.selectedRealMarker
              ]}>
          ...
          {/* Real Route Polylines for InstaPlan */}
          {instaPlanActive && routeSequence.length > 1 && (
            <Polyline
              coordinates={routeSequence.map(venue => ({
                latitude: venue.latitude,
                longitude: venue.longitude,
              }))}
              strokeColor="#8B5CF6"
  ...
  // Web implementation with enhanced features (fallback)
  return (
    <View style={styles.container}>
      <View style={[
